...
class DeleteReviewController
{
    ...
    {
        //access the review, find out what paper it was associated with
        $review = new Review();
        $review->connectDB();
    ...
    {
        //updates status of paper back tp pending
		$paper1 = new Paper();
    ...
    function deleteReview($rid)
    {
        ...
        //get the number of reviews on this paper
        $count = $result->num_rows;
        if($count >= 2){
            //if there is >=2 reviews on this paper, can safely delete the review
            if($review->deleteReview($rid)) return true;
            else return false;
        }
        else{
            //careful! delete review should also set paper back to pending status
            if($review->deleteReview($rid))
            {
                //if delete review is sucessful
